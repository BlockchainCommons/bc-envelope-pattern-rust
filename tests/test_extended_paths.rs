...
fn test_cbor_pattern_extended_paths() {
    // Test that `cbor` patterns now return extended paths that include the
    // internal `cbor` structure
...
fn test_cbor_pattern_extended_paths_nested_structure() {
    ...
    let nested_cbor =
        parse_dcbor_item(r#"{"name": "Alice", "scores": [95, 87, 92]}"#)
            .unwrap();
    ...
    assert_eq!(
        paths.len(),
        3,
        "Should find 3 numbers in the nested structure"
    );
    ...
    assert_actual_expected!(
        actual,
        expected,
        "Nested structure should show proper path extension with intermediate elements"
    );
}
...
fn test_cbor_pattern_no_extended_paths_for_non_pattern() {
    ...
    assert_actual_expected!(
        actual,
        expected,
        "`cbor` without pattern should return just the root envelope"
    );
    ...
    assert_actual_expected!(
        actual,
        expected,
        "`cbor` with exact value should return just the root envelope"
    );
}
