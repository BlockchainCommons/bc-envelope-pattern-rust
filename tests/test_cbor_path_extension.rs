...
fn test_cbor_pattern_text_search_paths() {
    ...
    assert_actual_expected!(
        actual,
        expected,
        "Text search should find all text elements with proper path extension"
    );
}
...
fn test_cbor_pattern_complex_nested_paths() {
    ...
    assert_actual_expected!(
        actual,
        expected,
        "Complex nested structure should show full path extension through all levels"
    );
}
...
fn test_cbor_pattern_map_key_value_paths() {
    ...
    for (i, path) in paths.iter().enumerate() {
        ...
        if let Some(cbor) = last_element.as_leaf() {
            assert!(
                matches!(cbor.as_case(), dcbor::CBORCase::Unsigned(_)),
                "Last element of path {} should be a number",
                i
            );
        }
    }
